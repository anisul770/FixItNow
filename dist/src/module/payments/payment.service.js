...
const getGateway = () => new SSLCommerzPayment(config.store_id, config.store_passwd, isLive);
const initilization = async (booking_id, userId) => {
    ...
    const data = {
        ...
        success_url: `${config.front_url}/api/payment/${booking_id}/success`,
        fail_url: `${config.front_url}/api/payment/${booking_id}/fail`,
        cancel_url: `${config.front_url}/api/payment/${booking_id}/cancel`,
...
const successPayment = async (booking_id, val_id) => {
    if (!val_id) {
...
const failPayment = async (booking_id) => {
    const payment = await prisma.payment.findUniqueOrThrow({
        where: {
            bookingId: booking_id
        }
...
const cancelPayment = async (booking_id) => {
    const payment = await prisma.payment.findUniqueOrThrow({
        where: {
            bookingId: booking_id
        }
...
const getPaymentDetails = async (booking_id, userId, role) => {
    const payment = await prisma.payment.findUniqueOrThrow({
        where: {
            bookingId: booking_id
        },
